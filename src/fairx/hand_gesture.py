"""
FAIRX Hand Gesture Detection Module
...
EBUF = EvidenceRecorder(fps=15, seconds=8, enabled=Config.ENABLE_EVIDENCE)
...
class HandGestureThread(threading.Thread):
    def __init__(self):
        ...
        self.cooldown = Config.event_cooldown.get("hand_gesture", 2.0)
            ...
            if results.multi_hand_landmarks:
                ...
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    if is_suspicious and now - self.last_event_time > self.cooldown:
                        SCORE.add_event("hand_gesture", conf)
                ...
                if num_hands >= 2:
                    ...
                    if is_passing and now - self.last_event_time > Config.event_cooldown.get("paper_passing", 3.0):
                        SCORE.add_event("paper_passing", conf)
                ...
                if len(self.hand_history) >= 10:
                    ...
                    if movement > Config.hand_movement_threshold:
                        if now - self.last_event_time > self.cooldown:
                            intensity = min(1.0, movement / (Config.hand_movement_threshold * 2))
                            SCORE.add_event("hand_gesture", intensity)
